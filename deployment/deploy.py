...
def _build_app():
    ...
    return AdkApp(
        ...
        env_vars={
            ...
            "LOG_DATASET": os.environ.get("LOG_DATASET", "logging_analytics"),
...
def _force_delete_engine(resource_name: str) -> None:
    """Delete a ReasoningEngine via REST with ?force=true to remove child sessions."""
    import google.auth  # noqa: PLC0415
    import google.auth.transport.requests  # noqa: PLC0415
    import requests as http_requests  # noqa: PLC0415

    ai_location = resource_name.split("/")[3]
    url = f"https://{ai_location}-aiplatform.googleapis.com/v1/{resource_name}?force=true"
    creds, _ = google.auth.default(scopes=["https://www.googleapis.com/auth/cloud-platform"])
    creds.refresh(google.auth.transport.requests.Request())
    resp = http_requests.delete(url, headers={"Authorization": f"Bearer {creds.token}"})
    resp.raise_for_status()


def _cleanup_old_engines(new_resource_name: str) -> None:
    """Delete all ReasoningEngines with the same display name except the newly created one."""
    from vertexai.preview import reasoning_engines  # noqa: PLC0415

    all_engines = reasoning_engines.ReasoningEngine.list()
    old = [e for e in all_engines
           if e.display_name == DISPLAY_NAME and e.resource_name != new_resource_name]
    ...
    for engine in old:
        print(f"  Deleting old engine: {engine.resource_name}")
        try:
            engine.delete()
            print(f"    Deleted.")
        except Exception as exc:  # noqa: BLE001
            if "child resources" in str(exc):
                try:
                    _force_delete_engine(engine.resource_name)
                    print(f"    Deleted (force=true).")
                except Exception as force_exc:  # noqa: BLE001
                    print(f"    WARNING: force delete also failed: {force_exc}")
            else:
                print(f"    WARNING: could not delete {engine.resource_name}: {exc}")


def deploy(args: argparse.Namespace) -> None:
    """Package and deploy the agent to Agent Engine, then remove old deployments."""
    ...
    remote_app = reasoning_engines.ReasoningEngine.create(
        ...
        description=(
            ...
            "revokes their licences, runs daily usage audits, and logs every action."
        ),
    ...
    print(f"\nCleaning up previous deployments with display name '{DISPLAY_NAME}' …")
    _cleanup_old_engines(remote_app.resource_name)
