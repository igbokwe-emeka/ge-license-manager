...
def test_get_parent_resources_success_temp(monkeypatch):
    monkeypatch.setenv("GCP_PROJECT_ID", "test-project")
    parent = _get_parent_resources()
    assert any("test-project" in p for p in parent)

def test_get_parent_resources_failure(monkeypatch):
...
def test_list_all_licensed_users_success(mock_client_init, monkeypatch):
    ...
    assert result["licensed_users"][0]["assigned_date"] == "N/A"
    assert result["licensed_users"][0]["last_used_date"] == "N/A"
...
def test_revoke_gemini_license_success(mock_client_init, monkeypatch):
    ...
    with patch("ge_governance_agent.tools.license_manager.get_user_license_status") as mock_status:
        mock_status.return_value = {"has_license": True, "state": "ASSIGNED", "error": None, "parent_resource": "projects/igbokwe/locations/global/userStores/default_user_store"}
