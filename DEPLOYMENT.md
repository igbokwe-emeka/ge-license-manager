...
| `roles/storage.objectCreator` | Write daily audit reports to GCS |
...
To allow the application to authenticate, generate a JSON key for the service account you created in Step 1.
...
Store the key file securely. Set the path in your `.env` (the setup script handles this automatically):
```
...
### Step 3 — Create a Linked BigQuery Dataset (Recommended)

To enable advanced querying and better performance, you can create a linked BigQuery dataset. You can do this manually via the console or **ask the agent to do it for you** once deployed:

> **Agent Prompt:** `"Setup Log Analytics and create a linked BigQuery dataset named logging_analytics."`

If you use a linked dataset, set the `LOG_DATASET` environment variable in your `.env` (see Section 5).
...
| `SUBSCRIPTION_IDS` | Comma-separated list of Project IDs holding the GE subscriptions (optional, defaults to GCP_PROJECT_ID) | `project-a,project-b` |
...
| `AUDIT_BUCKET` | GCS bucket for daily audit reports | `gs://my-project-audit-reports` |
| `LOG_DATASET` | Linked BigQuery dataset (optional) | `logging_analytics` |
...
| `BigQuery table not found` | Log Analytics not enabled or linked dataset missing | Run `gcloud logging buckets update --enable-analytics` or **ask agent to "Setup Log Analytics"** |
